...
def balanced(m):
    ...
    def sub_balanced(a):
        s = end(a)
        return True if is_planet(s) else balanced(s)
    def torque(a):
        return total_weight(end(a)) * length(a)
    return torque(left(m)) == torque(right(m)) and sub_balanced(left(m)) and sub_balanced(right(m))
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    True
    """
    return tree(lokis_replacement if is_leaf(t) and label(t) == 'loki' else label(t), [replace_loki_at_leaf(s, lokis_replacement) for s in branches(t)])
...
def has_path(t, word):
    ...
    return label(t) == word[0] and (len(word) == 1 or True in [has_path(b, word[1:]) for b in branches(t)])
